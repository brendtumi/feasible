import { debug, isAccessible, logger, readAsObject, write } from './index.js';
...
export class Lock {
  ...
  /**
   * Does the lock file need to be updated?
   * @returns {boolean}
   */
  isFileNeedUpdate () {
  ...
  async export (checksum, variables, files, version, lockFilePath) {
    debug(`Lock export: ${lockFilePath || this.lockFilePath}`);
    await write({
      checksum,
      variables,
      files
