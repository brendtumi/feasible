...
async function cloneRepository(configuration, variables) {
  const download = require('download-git-repo');
  const repository = replaceVariableInString(configuration.repository, variables);
  console.log(chalk.yellow(`Cloning ${repository}`));
  return new Promise((resolve, reject) => {
    download(repository, process.cwd(), { clone: true }, function (err) {
      if (err) {
        reject(err);
      } else {
        resolve();
