...
async function readConfigFile(fileGlob) {
  ...
  if (firstMatch) {
    try {
      ...
      switch (extname(firstMatch)) {
        case '.json': {
          return { configuration: JSON.parse(contents), file: firstMatch };
        }
        case '.json5': {
          const { parse } = require('json5');
          return { configuration: parse(contents), file: firstMatch };
        }
        case '.yml': {
          return { configuration: load(contents), file: firstMatch };
        }
      }
    } catch (e) {
      throw new Error(`Parse error on "${firstMatch}": ${e.message}`);
    }
    throw new Error(`Unsupported file format "${firstMatch}"`);
  }
  ...
  if (configuration && 'files' in configuration) {
    ...
    for (let [path, file] of Object.entries(configuration.files)) {
      ...
      if (typeof file === 'string') {
        file = replaceVariableInString(file, vars, separator);
        console.log(boxen(file, { padding: 1, borderColor: 'blue', borderStyle: 'round' }));
