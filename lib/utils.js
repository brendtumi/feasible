  ...
  for (const [name, value] of Object.entries(defaults)) {
    if (typeof value === 'object' && value.type === 'bash') {
      ...
      if (out.stderr.length > 0) {
        ...
        log.info(`Resolve variable @ ${name}: ${out.stdout}`);
...
async function executeFiles(configuration, variables, lock, { separator }) {
  ...
  const vm = new VM({
    ...
    eval: false,
    wasm: false,
    sandbox: Object.fromEntries(Object.entries(variables).map(([k, v]) => [`$${k}`, v])),
  });

  if (configuration && 'files' in configuration) {
    for (const [path, file] of Object.entries(configuration.files)) {
      if (typeof file === 'string') {
        const generated = replaceVariableInString(file, variables, separator);
        await makeDir(dirname(path));
        writeFileSync(path, generated);
        paths.push(path);
        log.aligned(path, 'File Generated.', '.');
      } else if (typeof file === 'object' && 'type' in file && 'variables' in file) {
        const generated = objectFileType(file, variables);
        await makeDir(dirname(path));
        writeFileSync(path, generated);
        paths.push(path);
        log.aligned(path, 'File Generated.', '.');
      } else if (typeof file === 'object' && 'condition' in file && 'content' in file) {
        let isTruthy = false;
        try {
          isTruthy = !!vm.run(file.condition);
        } catch (e) {
        ...
        if (isTruthy) {
          const generated =
            typeof file.content === 'object' && 'type' in file.content && 'variables' in file.content
              ? objectFileType(file.content, variables)
              : replaceVariableInString(file.content, variables, separator);
          ...
          log.aligned(path, 'Condition not met.', '.');
        }
      } else {
        log.warn(`Unrecognizable file type: "${path}"`);
      }
    }
  }
  return paths;
