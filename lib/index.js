...
const { readConfigFile, generateChecksum, promptUI, execHook, executeFiles } = require('./utils');
const Lock = require('./lock');

async function init({ config = 'feasible.{json,json5,yml}', force = false, parallel = false, separator = '=' }) {
  const { configuration, file } = await readConfigFile(config);
  ...
  if (execute) {
    ...
    if (preHooks) {
      await executeFiles(configuration, lock, { separator });
    } else {
      lock.restore();
    }
